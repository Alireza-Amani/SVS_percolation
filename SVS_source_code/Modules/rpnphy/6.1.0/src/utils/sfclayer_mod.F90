...
#include <msg.h>
  ...
  procedure(stability_function), pointer, save :: &
       sf_stable => null(), & !sf_stable_delage97, &                   !Stability functions (stable surface layer)
       sf_unstable => null() !sf_unstable_delage92                  !Stability functions (unstable surface layer)
...
! Alireza mod
    REAL, DIMENSION(1):: z0j, z0locj, z0tj, fmj, vaj, fcorj
    ...
    ROW: do j=1,n
       ...
       if (lmin > 0. .and. dthv > 0.) then
          ...
          hc = pblheight(zuj, z0j(1), vaj(1), ilmm, fcorj(1), lzz0j)
          sf => sf_stable
          ff_zm = (/zuj+z0rmj, z0j(1)/)
          ff_zh = (/ztj+z0rtj, z0tj/)
          call sf(fmj(1), fhj, lzz0j, lzz0tj, ilmm, hc, beta, &
               F_zm=ff_zm, F_zh=ff_zh)
          ribc = zp * ilmm * fhj / fmj(1)**2
          ! Wind speed is adjusted to keep Obukhov length above the minimum
          vlmin = sqrt( max(0.d0, 1./ribc * GRAV * zp * dthv/(tvs + 0.5*dthv)) )
       endif
       vaj = max(vaj, vlmin)
       ribj = GRAV / (tvs + 0.5*dthv) * zp * dthv/vaj(1)**2
       ...
       if (rineutral > 0. .and. z0j(1) < 0.01 .and. abs(ribj) < rineutral) ribj = 0.

       ! First guess of neutral stability function adjustments and Obukhov length
       ! uses the Delage (1997) stability functions
       if (ribj >= 0.)  then
          am = 2.5*D97_AS * ribj / max(2*z0j(1), 1.0)
          ah = 2.5*D97_AS * ribj / max(sqrt(z0j(1)*z0tj(1)), 1.0)
       else
       ...
       ITERATIONS: do it=1,itmax
          ...
          if(optz0 > 0) then
             ...
             lzz0j = log((z0rmj + zuj) / z0j(1))
             lzz0tj = log((z0rtj + ztj) / z0tj(1))
          endif
          ...
          hj  = pblheight(zuj, z0j(1), vaj(1), ilmoj, fcorj(1), fmj(1))
          ...
          ff_zm = (/zuj+z0rmj, z0j(1)/)
          ff_zh = (/ztj+z0rtj, z0tj/)
          call sf(fmj(1), fhj, lzz0j, lzz0tj, ilmoj, hj, beta, &
               F_zm=ff_zm, F_zh=ff_zh, &
               F_dfm=dfm, F_dfh=dfh)
          ...
          if (it < itmax) then
             g = ribj - fhj / (fmj(1) * fmj(1)) * zp * ilmoj
             dg = -fhj / (fmj(1)*fmj(1)) * zp * (1. + dfh/fhj - 2.*dfm/fmj(1))
       ...
       fm(j) = fmj(1)                              !Integrated momentum stability function
       ...
       va(j) = vaj(1)                              !Adjusted first-level wind speed

       ! Diagnose turbulent surface exhange properties
       cm = karman / fmj(1)                        !Momentum exchange coefficient
       ct = karman / fhj                        !Heat exchange coefficient
       ue(j) = cm * vaj(1)                         !Friction velocity
