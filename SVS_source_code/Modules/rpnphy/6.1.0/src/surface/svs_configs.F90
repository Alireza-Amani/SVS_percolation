module svs_configs
  ...
  ! variable to decide to read user defined params from the MESH_parameter.txt
   INTEGER :: read_parameters(1) = 1
   INTEGER :: save_minutes_csv(1) = 1

   ! print info inside inisoili
   LOGICAL :: print_inisoili = .TRUE.

   LOGICAL :: save_geoparams_to_file = .TRUE.
   INTEGER :: geoparam_file_unit = 3970

   LOGICAL :: save_modified_geoparams_to_file = .TRUE.
   INTEGER :: modified_geoparam_file_unit = 3971

   ! variables to help me print info only once within certain subroutines
   LOGICAL :: print_hydro_svs = .TRUE. ! inside hydro_svs.f90
   LOGICAL :: print_freezing_svs = .TRUE. ! inside soil_freezing.f90
   ...
   ! to be used inside hydro_svs.f90
   REAL :: user_wfcdp(1) = 666.0 ! should b ini to 666
   REAL :: user_maxpnd(1) = 666.0

   INTEGER :: user_OPT_LIQWAT(1) = 666
   INTEGER :: user_OPT_FRAC(1) = 666
   INTEGER :: user_OPT_SNOW(1) = 666

   INTEGER :: user_water_ponding(1) = 666
   INTEGER :: user_KFICE(1) = 666
   INTEGER :: user_WAT_REDIS(1) = 666
...
#include <msg.h>
  ...
  ! function to read parameters from MESH_parameters.txt
    FUNCTION read_param(parameter_name, value_length, print_overwrite_info) result(parameter_values)

      IMPLICIT NONE

      INTEGER, INTENT(in) :: value_length ! dimension of the variable vector
      CHARACTER (LEN = *), INTENT(in) :: parameter_name
      INTEGER :: ios1, istat, line_nr, iounit ! variables for the READ function
      LOGICAL :: found_it ! indicate whether the parameter is found inside the file
      CHARACTER (len = 2000) :: f_name , line_string
      REAL :: parameter_values (value_length) ! an array to store the read values for each parameter
      CHARACTER (LEN = LEN_TRIM(parameter_name)) :: keyword ! name of the parameter to look for, inside the file
      LOGICAL :: print_overwrite_info


      f_name = "MESH_parameters.txt" ! will read from this file
      keyword = parameter_name
      iounit = 666
      found_it = .FALSE.

      OPEN(unit=iounit, file = f_name, iostat=ios1, status="old", &
      position = "rewind", action="read")

      IF ( ios1 /= 0 ) STOP TRIM(f_name) // " does not exist buddy boy. Or I cant open it anyway." ! checking if the param file exist

      ! loop through them lines
      DO line_nr = 1, 200

         !  read the line into a string
         READ(unit=iounit, fmt="(A)", iostat=istat) line_string
         IF ( istat /= 0 ) EXIT

         line_string = ADJUSTL(TRIM(line_string))  ! remove the spaces

         ! some cleanups ----------
         ! skip comments and blank lines
         IF (line_string(1:1) .EQ. "!" .OR. line_string(1:1) .EQ. "") CYCLE

         ! cut the string right before the in-line comments
         IF (INDEX(line_string, "!") /= 0) THEN
               line_string = line_string(:INDEX(line_string, "!") - 1)
         END IF

         ! find the specific parameter
         IF (INDEX(ADJUSTL(line_string), parameter_name) == 1) THEN
                  found_it = .TRUE.

                  READ(line_string, *) keyword, parameter_values

                  IF (print_overwrite_info) THEN
                     WRITE(*, "(A)") "Overwriting " // keyword
                  END IF
         END IF

         IF (.NOT. found_it) THEN
            parameter_values = 666
         END IF
