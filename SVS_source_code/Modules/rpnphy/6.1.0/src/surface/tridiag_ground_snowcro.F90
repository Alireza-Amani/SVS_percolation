...
!*       0.2 declarations of local variables
!
REAL, DIMENSION(SIZE(PA,1) ,SIZE(PA,2), SIZE(PA,3)) :: ZW   ! work array
!REAL :: ZDET ! work array
REAL, DIMENSION(SIZE(PA,1) , SIZE(PA,3)) :: ZDET ! work array
!
!
...
DO JB = 1,INB
  ...
  DO JJ = 1,SIZE(PX,1)
    !
    ZDET(JJ,JB) = PB(JJ,1,JB)
    !
    PX(JJ,1,JB) = PY(JJ,1,JB) / ZDET(JJ,JB)
  ENDDO
ENDDO
!
DO JB = 1,INB
  !
  DO JL = 2,2*KMAX_USE(JB)
    !
    DO JJ = 1,SIZE(PX,1)
      !
      !
      ...
      IF ( JL<=KNLVLS_USE(JJ,JB)-KDIFLOOP ) THEN
        !
        ZW(JJ,JL,JB) = PC(JJ,JL-1,JB)  /ZDET(JJ,JB)
        ZDET(JJ,JB)  = PB(JJ,JL  ,JB) - PA(JJ,JL,JB) * ZW(JJ,JL,JB)
        !
        PX(JJ,JL,JB) = ( PY(JJ,JL,JB) - PA(JJ,JL,JB) * PX(JJ,JL-1,JB) ) / ZDET(JJ,JB)
        !
      ENDIF
      !
    ENDDO
...
DO JB = 1,INB
  !
  DO JL = 2*KMAX_USE(JB)-1,1,-1
    !
    DO JJ = 1,SIZE(PX,1)
      !
      IF ( JL<=KNLVLS_USE(JJ,JB)-1-KDIFLOOP ) THEN
        !
        PX(JJ,JL,JB) = PX(JJ,JL,JB) - ZW(JJ,JL+1,JB) * PX(JJ,JL+1,JB)
        !
      ENDIF
      !
    ENDDO
