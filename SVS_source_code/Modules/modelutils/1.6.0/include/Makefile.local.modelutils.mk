...
MODELUTILS_ABS        = yyencode yydecode yy2global time2sec_main flipit test_integrals testtiming test_inputio8
...
maintesttiming=testtiming.Abs
testtiming: | testtiming_rm $(BINDIR)/$(maintesttiming)
	ls -l $(BINDIR)/$(maintesttiming)
testtiming_rm:
	if [[ "x$(MODELUTILS_ABS_DEP)" == "x" ]] ; then \
		rm -f $(BINDIR)/$(maintesttiming) ; \
	fi
$(BINDIR)/$(maintesttiming): $(MODELUTILS_ABS_DEP) | $(MODELUTILS_VFILES)
	export MAINSUBNAME="testtiming" ;\
	export ATM_MODEL_NAME="$${MAINSUBNAME} $(BUILDNAME)" ;\
	export ATM_MODEL_VERSION="$(MODELUTILS_VERSION)" ;\
	export RBUILD_LIBAPPL="$(MODELUTILS_LIBS_V) $(MODELUTILS_LIBS_DEP)" ;\
	export RBUILD_COMM_STUBS="$(LIBCOMM_STUBS) $(MODELUTILS_DUMMYMPISTUBS)";\
...
maintest_inputio8=test_inputio8.Abs
test_inputio8: | test_inputio8_rm $(BINDIR)/$(maintest_inputio8)
	ls -l $(BINDIR)/$(maintest_inputio8)
test_inputio8_rm:
	if [[ "x$(MODELUTILS_ABS_DEP)" == "x" ]] ; then \
		rm -f $(BINDIR)/$(maintest_inputio8) ; \
	fi
$(BINDIR)/$(maintest_inputio8): | $(MODELUTILS_VFILES)
	export MAINSUBNAME="test_inputio8" ;\
