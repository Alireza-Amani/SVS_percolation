...
!todo: Replace 'cm' instance with 'vs' counterparts.
    ...
    integer :: iout_svs1_soil_min = 162

    integer :: iout_svs1_wb = 6661
    INTEGER :: iout_svs1_test = 6662
    ...
    ! Alireza
    REAL(10) :: acc_percolation(1) = 0.0
    REAL(10) :: acc_overlandflow(1) = 0.0
    REAL(10) :: acc_lateralflow(1) = 0.0
    REAL(10) :: acc_ET(1) = 0.0
    REAL(10) :: acc_precipitation(1) = 0.0
...
!end do
    ...
    ! hav to add drainage and ET and runoff and lateral flow 
    ...
    if(svs_mesh%vs%schmsol=='SVS2') then
   ...
   else if(svs_mesh%vs%schmsol=='SVS') then  ! here add column header for new variables
       ...
       write(iout_svs1_soil, FMT_CSV, advance = 'no') "TPERM", 'TGROUND_1','TGROUND_2','TVEG_1','TVEG_2', "WVEG"
       write(iout_svs1_soil, FMT_CSV, advance = 'no') "ACC_PCP", 'ACC_DRAI', "ACC_ET", "ACC_OVFLW"

    !    lets add all snow stuff into this output file,

       write(iout_svs1_soil, FMT_CSV, advance = 'no') 'SNOMA', 'SNODP','SNODEN','SNOALB','WSNO','TSNO_1','TSNO_2'
       write(iout_svs1_soil, FMT_CSV, advance = 'no') 'SNVMA', 'SNVDP','SNVDEN','SNVALB','WSNV','TSNV_1','TSNV_2'
       write(iout_svs1_soil, *)


    ! for minute scale output ************
    IF (save_minutes_csv(1) .NE. 0) THEN
        open(iout_svs1_soil_min, file = './' // trim(fls%GENDIR_OUT) // '/' // 'svs1_soil_minutes.csv', action = 'write')
        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') 'YEAR', 'JDAY', 'HOUR', 'MINS'
        do j = 1, nl_svs
            write(level, FMT_GEN) j
            write(iout_svs1_soil_min, FMT_CSV, advance = 'no') &
                                trim(VN_SVS_ISOIL) // '_' // trim(adjustl(level)), &
                                trim(VN_SVS_WSOIL) // '_' // trim(adjustl(level))
        end do
        if(svs_mesh%vs%lsoil_freezing_svs1) then 
        ...
        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') "TPERM", 'TGROUND_1','TGROUND_2','TVEG_1','TVEG_2', "WVEG"
        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') "ACC_PCP", 'ACC_DRAI', "ACC_ET", "ACC_OVFLW"

        !    lets add all snow stuff into this output file,

        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') 'SNOMA', 'SNODP','SNODEN','SNOALB','WSNO','TSNO_1','TSNO_2'
        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') "WATPOND" 
        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') 'SNVMA', 'SNVDP','SNVDEN','SNVALB','WSNV','TSNV_1','TSNV_2'
        write(iout_svs1_soil_min, *)

    END IF ! if cond for write min csv file 
...
#define z1(n) z2(n,0)
    ...
    subroutine runsvs_mesh_within_tile(shd, fls, cm)
        ...
        ! vincent added modification: branch `test_alireza` 
        ...
        ! Alireza 
        acc_percolation = acc_percolation + max(0.0, busptr(vd%watflow%i)%ptr((khyd*ni + 1):(khyd + 1)*ni, trnch))
        acc_overlandflow = acc_overlandflow + max(0.0, busptr(vd%runofftot%i)%ptr(((indx_soil - 1)*ni + 1):indx_soil*ni, trnch))
        acc_ET = acc_ET + busptr(vd%wflux%i)%ptr(:, trnch) * ic%dts
        acc_precipitation = acc_precipitation + (vs%tile%pre * ic%dts)
...
1010    format(9999(g15.7e2, ','))
        ...
        if(svs_mesh%vs%schmsol=='SVS2') then
           ...
           if (ic%now%mins ==0) then! Full hour
              ...
              if ( modulo(ic%now%hour,nfreq) == 0 .and. svs_mesh%vs%lout_snow_profile ) then
                ...
                IF (save_minutes_csv(1) .NE. 0) THEN
                    ...
                    ! Write file containing soil outputs
                    write(iout_svs1_soil_min, FMT_CSV, advance = 'no') ic%now%year, ic%now%jday, ic%now%hour, ic%now%mins
                    do i = 1, nl_svs
                        write(iout_svs1_soil_min, FMT_CSV, advance = 'no') &
                            busptr(vd%isoil%i)%ptr(((i - 1)*ni + 1):i*ni, trnch) , &
                            busptr(vd%wsoil%i)%ptr(((i - 1)*ni + 1):i*ni, trnch)
                    ...
                    write(iout_svs1_soil_min, FMT_CSV, advance = 'no') busptr(vd%tperm%i)%ptr(1:ni, trnch), &
                                                                        busptr(vd%tground%i)%ptr(1:ni, trnch), & 
                                                                        busptr(vd%tground%i)%ptr((ni+1):2*ni, trnch), &
                                                                        busptr(vd%tvege%i)%ptr(1:ni, trnch), &
                                                                        busptr(vd%tvege%i)%ptr(ni+1:2*ni, trnch), & 
                                                                        busptr(vd%wveg%i)%ptr(1:ni, trnch)

                    write(iout_svs1_soil_min, FMT_CSV, advance = 'no') acc_precipitation, acc_percolation, acc_ET, acc_overlandflow

                    ! The added snow outputs 

                    write(iout_svs1_soil_min, FMT_CSV, advance = 'no') busptr(vd%snoma%i)%ptr(:, trnch),busptr(vd%snodpl%i)%ptr(:, trnch), &
                                busptr(vd%snoden%i)%ptr(:, trnch), busptr(vd%snoal%i)%ptr(:, trnch),busptr(vd%wsnow%i)%ptr(:, trnch), &                    
                                busptr(vd%tsnow%i)%ptr(1:ni, trnch),busptr(vd%tsnow%i)%ptr((ni+1):2*ni, trnch)

                    write(iout_svs1_soil_min, FMT_CSV, advance = 'no') busptr(vd%watpond%i)%ptr(1:ni, trnch)      

                    write(iout_svs1_soil_min, FMT_CSV, advance = 'no') busptr(vd%snvma%i)%ptr(:, trnch),busptr(vd%snvdp%i)%ptr(:, trnch), &
           ...
           if (ic%now%mins ==0) then! Full hour
              ...
              write(iout_svs1_soil, FMT_CSV, advance = 'no')     busptr(vd%tperm%i)%ptr(1:ni, trnch), &
                                                                 busptr(vd%tground%i)%ptr(1:ni, trnch), & 
                                                                 busptr(vd%tground%i)%ptr((ni+1):2*ni, trnch), &
                                                                 busptr(vd%tvege%i)%ptr(1:ni, trnch), &
                                                                 busptr(vd%tvege%i)%ptr(ni+1:2*ni, trnch), & 
              ...
              write(iout_svs1_soil, FMT_CSV, advance = 'no') acc_precipitation, acc_percolation, acc_ET, acc_overlandflow

              ! The added snow outputs 

              write(iout_svs1_soil, FMT_CSV, advance = 'no') busptr(vd%snoma%i)%ptr(:, trnch),busptr(vd%snodpl%i)%ptr(:, trnch), &
                        busptr(vd%snoden%i)%ptr(:, trnch), busptr(vd%snoal%i)%ptr(:, trnch),busptr(vd%wsnow%i)%ptr(:, trnch), &                    
                        busptr(vd%tsnow%i)%ptr(1:ni, trnch),busptr(vd%tsnow%i)%ptr((ni+1):2*ni, trnch)
              write(iout_svs1_soil, FMT_CSV, advance = 'no') busptr(vd%snvma%i)%ptr(:, trnch),busptr(vd%snvdp%i)%ptr(:, trnch), &
                        busptr(vd%snvden%i)%ptr(:, trnch), busptr(vd%snval%i)%ptr(:, trnch),busptr(vd%wsnv%i)%ptr(:, trnch), &                    
                        busptr(vd%tsnowveg%i)%ptr(1:ni, trnch),busptr(vd%tsnowveg%i)%ptr((ni+1):2*ni, trnch)
